package com.gaoan.forever.utils.ftp;
...
public class SftpUtils implements Serializable{
	 ...
	 * @return
	 */
	public boolean moveTo(String oldpath, String newpath) {
		boolean ret = false;
		try {
	...
	public static void main(String[] args) {
		String sftp_ip = "192.168.177.131";
		int sftp_port = 22;
		String sftp_username = "root";
		String sftp_password = "nick";
		String sftp_workdir = "/test";
		SftpUtils t = new SftpUtils(sftp_ip, sftp_port, 10000, sftp_username, sftp_password);
		try {
			System.out.println("开始");
			t.login();
			boolean changeDir = t.changeDir(sftp_workdir);
			System.out.println("changeDir=" + changeDir + "," + t.currentDir());
			// String[] lsFiles = t.lsFiles();
			// for (int i = 0; i < lsFiles.length; i++) {
			// System.out.println(lsFiles[i]);
			// }
			// // String pathName = "/";// 当前目录;
			// String pathName = t.currentDir();// 当前目录;
			// String fileName = "5.txt";
			// String localFile = "d://5.txt";
			// boolean uploadFile = t.uploadFile(pathName, fileName, localFile);
			// System.out.println("上传成功否？===" + uploadFile);
			boolean moveTo = t.moveTo("test1.txt", "/test/test2.txt");
	...
	public String toString() {
		return "SftpUtils [logger=" + logger + ", session=" + session + ", channel=" + channel + ", host=" + host
				+ ", port=" + port + ", timeout=" + timeout + ", username=" + username + ", password=" + password + "]";
	}
